...
BASIC USAGE:
    ...
    python tbp_apple_silicon.py input_video.mp4 output_folder/
    
    # With dimension swap (if output looks scrambled)
    python tbp_apple_silicon.py input_video.mp4 output_folder/ --swap-dimensions
    
    # Create output video from panoramas
    python tbp_apple_silicon.py input_video.mp4 output_folder/ --make-video
    
    # Frame interpolation for low-fps videos
    python tbp_apple_silicon.py input_video.mp4 output_folder/ --interpolate 4
...
DEPENDENCIES:
    ...
    Optional:
        ...
        - rife-ncnn-vulkan : Neural network frame interpolation
...
class FrameInterpolator:
    ...
    Supports three backends:
        - FFmpeg minterpolate: Fast, good quality, no extra dependencies
        - RIFE: Best quality, uses neural networks, requires rife-ncnn-vulkan
    ...
    def __init__(self, method: str = 'auto'):
        ...
        Args:
            method: Interpolation method to use. Options:
                    - 'auto': Use RIFE if available, otherwise FFmpeg
        ...
        # Warn if RIFE requested but not available
        if method == 'rife' and not HAS_RIFE:
            print("WARNING: RIFE not available, falling back to FFmpeg")
            print("         Install with: pip install rife-ncnn-vulkan")
            self.method = 'ffmpeg'
        
        # Initialize RIFE model if using that method
        if self.method == 'rife' and HAS_RIFE:
            # gpuid=0 uses the first GPU (Apple Silicon integrated GPU)
            self.rife = Rife(gpuid=0)
    
    def interpolate_video_ffmpeg(self, input_path: str, output_path: str, 
                                  multiplier: int = 2) -> str:
        """
        Interpolate video using FFmpeg's minterpolate filter.
        
        This method creates a new video file with interpolated frames.
        It uses motion-compensated interpolation (MCI) with adaptive
        overlapped block motion compensation (AOBMC).
        ...
        """
        # First, probe the input video to get its frame rate
        probe_cmd = [
            'ffprobe', '-v', 'error',
            '-select_streams', 'v:0',
        ...
        # Parse frame rate (can be fraction like "30000/1001" or decimal)
        if '/' in fps_str:
        ...
        print(f"Interpolating {input_fps:.1f} fps → {target_fps:.1f} fps using FFmpeg minterpolate...")
        
        # Build FFmpeg command with minterpolate filter
    ...
    def interpolate_frames_rife(self, frames: np.ndarray, multiplier: int = 2) -> np.ndarray:
        ...
        """
        if not HAS_RIFE:
            raise RuntimeError("RIFE not available - install with: pip install rife-ncnn-vulkan")
...
class AppleSiliconProcessor:
    ...
    def __init__(self, input_file: str, output_dir: str, slice_width: int = 1,
        ...
        # Resolve 'auto' interpolation method to actual method
        if interpolate_method == 'auto':
            if HAS_RIFE:
                self.interpolate_method = 'rife'
            else:
    ...
    def _preprocess_video_interpolation(self):
        ...
        interpolator.interpolate_video_ffmpeg(self.input_file, interpolated_path, self.interpolate)
...
EXAMPLES:
  Basic usage (GPU mode, single-threaded):
    python tbp_apple_silicon.py video.mp4 output/

  Multi-threaded CPU mode (often faster for many panoramas):
    python tbp_apple_silicon.py video.mp4 output/ --no-gpu --threads 8

  Fix scrambled output (dimension issues):
    python tbp_apple_silicon.py video.mp4 output/ --swap-dimensions
    python tbp_apple_silicon.py video.mp4 output/ --rotate 90

  Frame interpolation for low-fps videos (wider output):
    python tbp_apple_silicon.py video.mp4 output/ --interpolate 2
    python tbp_apple_silicon.py video.mp4 output/ --interpolate 4 --interpolate-method rife

  Create video and clean up images:
    python tbp_apple_silicon.py video.mp4 output/ --make-video --cleanup

  Benchmark to find fastest backend:
    python tbp_apple_silicon.py video.mp4 output/ --benchmark

INTERPOLATION METHODS:
  auto    : Uses RIFE if available, else FFmpeg (default)
  ffmpeg  : FFmpeg minterpolate filter (fast, good quality)
  rife    : RIFE neural network (best quality, needs: pip install rife-ncnn-vulkan)
...
PERFORMANCE TIPS:
  ...
  - Hardware video decoding (default) is typically 3-5x faster than software

INSTALLATION:
  Required: pip install numpy pillow tqdm av
  Optional: pip install mlx rife-ncnn-vulkan
...
For more information, see the docstring at the top of this script.
    ...
    print(f"RIFE (interpolation): {'Available' if HAS_RIFE else 'Not installed (pip install rife-ncnn-vulkan)'}")
    print()
